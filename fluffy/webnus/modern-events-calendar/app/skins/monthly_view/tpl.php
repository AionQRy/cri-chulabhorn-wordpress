...
if($this->next_previous_button)
{
    ...
    $navigator_html .= '<div class="mec-calendar-header"><h2>'.esc_html($this->main->date_i18n('F', $current_month_time).' '.$this->main->date_i18n('Y', $current_month_time)+$add_year).'</h2></div>';
...
jQuery(document).ready(function()
{
    jQuery("#mec_monthly_view_month_'.esc_js($this->id).'_'.date('Ym', $current_month_time).'").mecMonthlyView(
    {
        ...
        ajax_url: "'.admin_url('admin-ajax.php').'?lang='.$yp_locale.'",
...
else
{
    ...
    $div_end_topsec   = '</div>';
    $events_side      = '<div class="mec-calendar-events-side mec-clear"><h4 class="mini-title">';
    if (get_locale()=='th') {
      $text = 'ปฏิทิน';
    }
    else {
    ...
    $events_side      .= $text;
    $events_side      .= '</h4><div class="mec-month-side" id="mec_month_side_'.esc_attr($this->id).'_'.date('Ym', $current_month_time).'">'.$this->events_str.'</div></div>';
