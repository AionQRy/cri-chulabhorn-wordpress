...
foreach($this->weeks as $week_number => $week)
{
    ...
    $topWeeks .= '<dl class="mec-weekly-view-week bbb" id="mec_weekly_view_top_week_'.$this->id.'_'.date('Ym', strtotime($first_week_day)).$week_number.'" data-week-id="'.date('Ym', strtotime($first_week_day)).$week_number.'" data-week-number="' . $week_number . '" data-max-weeks="'.count($this->weeks).'">';
    foreach($week as $day) {
        $time = strtotime($day);
...
foreach($this->weeks as $week_number=>$week)
{
    ...
    $weeks .= '<dl class="mec-weekly-view-week aaa" id="mec_weekly_view_week_'.$this->id.'_'.date('Ym', strtotime($first_week_day)).$week_number.'" data-week-id="'.date('Ym', strtotime($first_week_day)).$week_number.'" data-week-number="' . $week_number . '" data-max-weeks="'.count($this->weeks).'">';
    foreach($week as $day)
    {
        $time = strtotime($day);
        $count = isset($this->events[$day]) ? count($this->events[$day]) : 0;
        ...
        $weeks .= '<dt data-events-count="'.$count.'" class="'.$activeClass.((strtotime($day) < $current_month_time or strtotime($day) >= $_1month_after)? 'mec-weekly-disabled' : '').'">'
